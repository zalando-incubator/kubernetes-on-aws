...
The deployment of skipper-ingress takes a bit more time than other components for safety reasons. To speedup deployment and reduce toil we will have skipper-ingress-operator in the future to update automatically skipper-ingress deployment in case skipper-ingress-canary is running in some bounds of "good" (think of errors and latency compared to skipper-ingress) in the cluster.
